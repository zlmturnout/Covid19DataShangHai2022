...
# **************************************VerTicaL@zlm**************************************
    ...
    def matplotlib_lineplot(self):
        ...
        if x_axis in ['Date','date','timestamp'] and y_axis in ['Date','date','timestamp']:
            x_pd_temp=pd.Series(x_list)
            x_pd_data=pd.to_datetime(x_pd_temp)
            y_pd_temp=pd.Series(y_list)
            y_pd_data=pd.to_datetime(y_pd_temp)
        elif x_axis not in ['Date','date','timestamp'] and y_axis in ['Date','date','timestamp']:
            x_pd_data=pd.Series(x_list)
            y_pd_temp=pd.Series(y_list)
            y_pd_data=pd.to_datetime(y_pd_temp)
        elif x_axis in ['Date','date','timestamp'] and y_axis not in ['Date','date','timestamp']:
            x_pd_temp=pd.Series(x_list)
            x_pd_data=pd.to_datetime(x_pd_temp)
            y_pd_data=pd.Series(y_list)
        else:
            x_pd_data=pd.Series(x_list)
            y_pd_data=pd.Series(y_list)
            
        self.plot_sql_data(x_pd_data,y_pd_data,x_axis,y_axis)
